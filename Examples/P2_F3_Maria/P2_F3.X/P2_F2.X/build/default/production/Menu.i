...
static char menuState;
static char pointer, pointerMarquee, auxPM;
...
void menuMotor(void){
  ...
  case 7:
            if(numUsers!=pointerUser)
                if(!(users[pointerUser].username[pointer]==users[numUsers].username[pointer])){
                    ++pointerUser;
                    ...
                    if(users[pointerUser].username[pointer]==users[numUsers].username[pointer])
                        if(users[numUsers].username[pointer]=='\0'){
                            pointer = 0;
                            ++menuState;
   ...
   break;
    ...
    if(users[pointerUser].password[pointer]==users[numUsers].password[pointer])
     if(users[numUsers].password[pointer]=='\0'){
  ...
  case 9:
            if(numUsers!=pointerUser){
                if(!(users[numUsers].username[pointer]==users[pointerUser].username[pointer])){
                    ++pointerUser;
   ...
   break;
        ...
        case 110:
            if((pointer) < 16){
                ...
                if(pointer+pointerMarquee > (29 -1)){
  ...
  case 12:
            if(TiGetTics(timerMenu) > 500){
                ...
                }else if(getPositionJoystick() == 'S' && getCentreJoystick()==0){
                ...
                }else if(getPositionJoystick() == 'W' && getCentreJoystick()==0){
                    resetPositionJoystick();
                    menuState = 10;
                    pointer=0;
                    LcClear();
  ...
  case 18:
   ...
   break;
        case 28:
            if(isPressed() && TiGetTics(timerMenu)> 24){
                TiResetTics(timerMenu);
                keyValue = KeyGetNumber();
        ...
        case 29:
            if(KeyGetValue() && TiGetTics(timerMenu)> 24){
                TiResetTics(timerMenu);
                keyValue = KeyGetNumber();
                minutes += keyValue;
                LcPutChar(keyValue + '0');
        ...
        case 32:
            if(!((PORTBbits.RB1) && (PORTBbits.RB2) && (PORTBbits.RB3))==1)
                if(KeyGetSMS()=='#'){
                    LcClear();
                    LcGotoXY(0,0);
                    option = 0;
   ...
   break;
        ...
        case 33:
            if(TiGetTics(timerMenu)>24 && !((PORTBbits.RB1) && (PORTBbits.RB2) && (PORTBbits.RB3))==0){
                menuState = aux;
                TiResetTics(timerMenu);
            }
            break;
   ...
   break;
        ...
        case 46:
            if(isPressed() && TiGetTics(timerMenu)> 1200){
    if(KeyGetSMS()== '#'){
                    stopTimerPic();
                    option = 0;
                    LcClear();
